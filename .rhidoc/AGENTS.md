...
in your `AGENTS.md` instead. `CLAUDE.md` is a compatibility link for Claude Code.
...
Headless agents and tools that do not receive a Skill tool will not see project
skills via system reminders. When work matches a skill, read its `SKILL.md`
directly from the project's neutral skill directory. The `rhidoc` CLI itself works everywhere — `rhidoc`
