...
4. **Report** — Tell the user:
   ...
   - Wait for completion: `bash .agents/skills/todo-task/launch.sh {slug} --watch` (foreground, blocks and exits with the outcome) — or, to watch an already-launched run without relaunching it, `bash .agents/skills/todo-task/wait.sh {slug}` in a background shell
