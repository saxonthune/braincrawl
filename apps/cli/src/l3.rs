...
//! and domain edges that point at canonical ids in the store's L1/L2 layers.
//! These commands edit the docs behind the machine-local store directly: plain
//! markdown files — one `<doc>.l3.md` per item — consolidated under a single,
//! config-driven root that the local server also serves over `/api/l3/docs`.
//! Carrying docs to another store is `store sync` (rules in `crates/l3-sync`).
//!
