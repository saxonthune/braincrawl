...
/// Parse a CLI-supplied work id into its alias scheme and value.
...
pub fn parse(id: &str) -> Option<Alias> {
    ...
    for prefix in DOI_URL_PREFIXES {
        if let Some(rest) = id.strip_prefix(prefix) {
            return Some(Alias { scheme: "doi".to_string(), value: rest.to_string() });
        }
    }

    if let Some((ns, value)) = id.split_once(':') {
        return Some(Alias { scheme: ns.to_lowercase(), value: value.to_string() });
    }

    if id.starts_with("10.") && id.contains('/') {
        return Some(Alias { scheme: "doi".to_string(), value: id.to_string() });
    }

    None
}

/// Pull the value of one alias scheme out of a store work record.
/// The store returns aliases as [{scheme, value}] objects.
pub fn alias_of(work: &serde_json::Value, ns: &str) -> Option<String> {
    let aliases = work.get("aliases").and_then(|a| a.as_array())?;
    for alias in aliases {
        if alias.get("scheme").and_then(|n| n.as_str()) == Some(ns) {
